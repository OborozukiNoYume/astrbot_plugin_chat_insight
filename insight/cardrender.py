...
def build_summary_card_data(group_name: str, group_id, s: dict, tz: ZoneInfo) -> dict:
    """群活跃度总览（service.summary 返回值）→ 复用群报卡片模板（无发言榜区块）。"""
    return {
        "group_name": _esc(group_name),
        "group_id": str(group_id),
        "title_badge": "群活跃度总览",
...
def build_trend_card_data(group_name: str, group_id, r_label: str, span: str,
                          days, tz: ZoneInfo) -> dict:
    """按天趋势（service.trend 返回值，空白天已补零）→ 逐日柱状图数据。"""
    mx = max((d.messages for d in days), default=0) or 1
    n = len(days)
    step = max(1, (n + 7) // 8)  # x 轴约 8 个日期刻度
    items = [
        {
            "date": d.date,
            "count": d.messages,
            "users": d.active_users,
            "hpx": round(d.messages / mx * 96) + (2 if d.messages else 0),
            "peak": d.messages == mx and mx > 1,
            "tick": d.date[5:] if i % step == 0 else "",  # MM-DD
        }
        for i, d in enumerate(days)
    ]
    peak_day = max(days, key=lambda d: d.messages) if days else None
    return {
        "group_name": _esc(group_name),
        "group_id": str(group_id),
        "range_label": r_label,
        ...
        "days": items,
        "stats": [
            {"label": "总消息", "value": f"{sum(d.messages for d in days):,}", "sub": ""},
            {"label": "峰值日", "value": str(peak_day.messages if peak_day else 0),
             "sub": peak_day.date if peak_day else ""},
            {"label": "日均", "value": f"{sum(d.messages for d in days) / n:.1f}" if n else "0",
             "sub": f"{n} 天"},
            {"label": "活跃天数", "value": str(sum(1 for d in days if d.messages)),
             "sub": "有消息的天"},
        ],
    }


def build_hours_card_data(group_name: str, group_id, r_label: str, span: str,
                          buckets: list[int], tz: ZoneInfo) -> dict:
    """24 小时分布（service.hours 返回值）→ 逐时柱状图数据。"""
    total = sum(buckets)
    mx = max(buckets) or 1
    peak = sorted(sorted(range(24), key=lambda h: -buckets[h])[:3])
    hours = [
        {
            "h": h,
            "count": c,
            "hpx": round(c / mx * 72) + (2 if c else 0),
            "peak": h in peak and total > 0,
            "tick": f"{h:02d}" if h % 3 == 0 else "",
        }
        for h, c in enumerate(buckets)
    ]
    ...
    return {
        ...
        "generated_at": datetime.now(tz).strftime("%Y-%m-%d %H:%M"),
        "hours": hours,
        "peak_hours": "/".join(f"{h:02d}" for h in peak) or "-",
        "stats": [
            {"label": "总消息", "value": f"{total:,}", "sub": ""},
            {"label": "高峰时段", "value": "/".join(f"{h:02d}" for h in peak) or "-",
             "sub": "Top3 小时"},
            {"label": "白天 06–18", "value": _pct(day_ratio), "sub": ""},
            {"label": "夜间", "value": _pct(1 - day_ratio), "sub": ""},
        ],
    }
...
async def render_summary_card(star, group_name: str, group_id, s: dict, tz) -> str | None:
    return await _render_card(
        star, "report_card", build_summary_card_data(group_name, group_id, s, tz), "总览"
    )


async def render_trend_card(star, group_name: str, group_id, r_label, span, days, tz) -> str | None:
    return await _render_card(
        star, "trend_card", build_trend_card_data(group_name, group_id, r_label, span, days, tz),
        "趋势",
    )


async def render_hours_card(star, group_name: str, group_id, r_label, span, buckets, tz) -> str | None:
    return await _render_card(
        star, "hours_card", build_hours_card_data(group_name, group_id, r_label, span, buckets, tz),
        "时段",
    )
